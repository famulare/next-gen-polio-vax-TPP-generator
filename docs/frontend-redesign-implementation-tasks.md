...
**Status:** ready for execution; no redesign implementation has started.
...
- [ ] Read the authorities above and `docs/scientific-web-widget-playbook.md`.
- [ ] Confirm the worktree is clean and record the starting commit in
  `IMPLEMENTATION_LOG.md`.
- [ ] Run `npm run verify` without changing generated output.
- [ ] Open the committed artifact at desktop and 360 px widths; record the
  current first viewport, setting surface, empty frontier, console state, and
  keyboard behavior in the implementation log.
- [ ] Record the current default outputs as legacy context only:
...
- [ ] Replace the misleading bundled-global-envelope ownership with two named
  objects:
...
- [ ] Store the display domain under a semantically explicit manifest field;
  do not retain a field named `envelope` if it no longer controls success.
- [ ] Bump every changed manifest/schema version and include it in scientific
  identity hashing.
- [ ] Change the default setting probe to UP/Bihar without coupling later probe
  changes to the decision scope.
- [ ] Remove `N_s=40` from the runtime display grid and default labels.
- [ ] Add an explicit migration or visible rejection for legacy URL state; do
...
- [ ] Keep `ScenarioV1.envelope` as the active point or rectangular decision
  scope; represent a named point as a degenerate envelope.
- [ ] Make `buildSettingSurface` use the versioned display domain rather than
  the active decision envelope.
- [ ] Keep `rLocEnvelopeMax`, frontier classification, and comparator status
  evaluated over the active decision scope.
- [ ] Preserve direct evaluation for status; raster and contour interpolation
  remain display-only.
- [ ] Ensure selected probes are view/inspection state and cannot mutate the
  decision scope without an explicit scope action.

### 1.3 Add numerical and schema tests

- [ ] Assert default direct UP/Bihar `R_loc=0.9201071208363125` within relative
  `1e-10` and selected-design pass status.
- [ ] Assert 92 passing product-grid points and eight Pareto points.
- [ ] Assert 81 exposure columns, 20 contact rows, 1,620 unique surface cells,
  and no cell with `N_s>20`.
- [ ] Assert a custom rectangular decision scope still uses its direct upper
  corner after monotonicity checks.
- [ ] Assert changing only the setting probe does not change pass/fail,
  frontier, model identity, or export identity.
- [ ] Assert changing decision scope invalidates and recomputes every dependent
  result.
- [ ] Cover missing, unknown, stale-version, and unit-mismatched manifest and
  URL inputs.
...
- [ ] Create `src/ui/presentation.ts` with typed pass, fail, tie,
  empty-frontier, unavailable-uncertainty, stale, invalid, and failed branches.
- [ ] Create explicit scientific, view, release, and transaction state types.
- [ ] Centralize labels for candidate, schedule, probe, decision scope,
  criterion, exact-versus-grid identity, and fixed comparators.
- [ ] Implement default hardest-known-anchor prose and its adjacent caveat:
  clearing UP/Bihar supports likely adequacy under less demanding modeled
  conditions but does not prove control everywhere.
- [ ] Keep the prior committed result visible and marked stale during
  recomputation; disable export until the new state commits atomically.
- [ ] Ensure hover/focus/chapter/drawer state does not change scientific or
  export identity.
- [ ] Add unit tests that derive every prose branch from model outputs rather
...
- [ ] Implement the chapter order from the redesign plan:
...
- [ ] Put the result and sufficiency qualification before advanced controls.
- [ ] Replace six peer metric cards with one authoritative result and a
  subordinate acquisition/shedding/index reading sequence.
- [ ] Implement the paper/ink typography, restrained russet accent, and single
  blue-white-red analytical scale without external fonts or assets.
- [ ] Remove unnecessary hard edges, shadows, color roles, and rounded-card
  nesting.
- [ ] Add semantic landmarks, skip navigation, coherent headings, and a
  responsive 360 px composition.
- [ ] Keep primary controls compact: product, schedule, assessment, probe, and
...
- [ ] Render the fixed `log10(R_loc)` domain `[-2,2]` using blue `#2166ac` ->
  near-white `#f7f7f2` at `R_loc=1` -> red `#b2182b`.
- [ ] Remove raster-cell strokes and prevent contours from tracing the plot
  frame as a scientific threshold.
- [ ] Add a non-color pass/fail cue, direction-of-pressure label, exact
  readout, and accessible text alternative.
- [ ] Mark UP/Bihar as the default decision anchor, not merely another point.
- [ ] Show any selected custom decision scope independently from the fixed
  display-domain boundary.
- [ ] Rework anchor labels with collision handling.
- [ ] Label the Matlab segment as the 3.2-61.7 micrograms/day household
  interval, daily basis, hybrid mapping, with inherited social link.
- [ ] State in screen and SVG export that status is direct over the decision
...
- [ ] Introduce acquisition blocking and breakthrough infectious-shedding
  reduction before showing effect space.
- [ ] Introduce biological take context and latent mean mucosal boost before
  showing product space.
- [ ] Render the same 2,601 direct evaluations in both coordinate systems.
- [ ] Show the default 92 passing points and eight-point Pareto boundary; draw
  no Pareto line when `frontier.pareto` is empty under another scope.
- [ ] Replace dense open-circle/hatch noise with a quiet evaluated field,
  threshold contour, selection, and comparator marks.
- [ ] Link hover, focus, persistent selection, and keyboard traversal across
  both views.
- [ ] Keep the selected exact design distinct from its nearest grid point.
- [ ] Keep Sabin 2 fixed and omit IPV from undefined take/boost coordinates.
- [ ] Make “Use this design” the only action that promotes a view selection to
...
- [ ] Add the index -> household member -> close-social-contact motif and state
  why `R_loc` is not complete-population `R_e`.
- [ ] Present `q_acq`, `q_shed`, `q_index`, and direct `R_loc` in causal-reading
  order while keeping `q_index` visibly diagnostic.
- [ ] Build the measurement/provenance map specified in the redesign plan.
- [ ] Label each quantity as scenario input, product property,
  calibrated/inherited, fixed v1 assumption, derived, or evidence gap.
- [ ] Explicitly distinguish take from receipt/coverage and latent
  OPV-equivalent mucosal immunity from measured serum titer.
- [ ] Defer one-dimensional slices unless usability review still finds take,
...
- [ ] Make probe and decision-scope controls visually and semantically
  independent.
- [ ] Support UP/Bihar default, other named singleton scopes, and explicit
  custom rectangular scopes.
- [ ] Move alpha, beta, dose, exposure/contact details, and scope bounds into
  scientifically grouped advanced controls.
- [ ] Present fixed values as assumptions, not disabled form fields.
- [ ] Preserve fail-closed validation and visible invalid/stale URL handling.
- [ ] Ensure canonical URL, JSON, CSV, SVG, visible state, and model identity
  agree after every scientific change.
- [ ] Make each SVG standalone with product, schedule, decision scope,
  criterion, qualification, legend, selected state, and interpolation note.
- [ ] Add visible Share and export success/failure feedback.
...
- [ ] Test keyboard-only operation, focus order, focus visibility, Escape
  behavior, and plot traversal.
- [ ] Test equivalent hover, focus, and touch readouts.
- [ ] Add concise live announcements for committed result changes.
- [ ] Test desktop, tablet, 360 px, 200% zoom, reduced motion, grayscale, high
  contrast, print, file URL, and GitHub Pages prefix behavior.
- [ ] Confirm no runtime request, external asset, console error, or random
  sampling.
- [ ] Update browser-smoke assertions for the new opening result, 20-row
  surface, scope controls, linked selection, and empty-frontier branch.
- [ ] Update README screenshots/text, provenance, `IMPLEMENTATION_LOG.md`,
  manifests, and committed `dist/index.html`.
- [ ] Run focused checks after each task, then final `npm run verify`.
- [ ] Rebuild twice and confirm no committed artifact diff.
- [ ] Perform the redesign plan's fatal-defect and quality-rubric audit.
