 ...
 * decision scope's link frequencies. A two-dimensional linked-exposure slice
...
function patchSurfaceScope(doc: Document): void {
  ...
  const dIh = scenario.envelope.dIhMax;
  const dHs = scenario.envelope.dHsMax;
  const signature = `${scope.id}:${dIh}:${dHs}`;
  ...
  const compatibleIds = new Set(SETTING_ANCHORS
    .filter((anchor) => sameFrequency(anchor.dIh.value, dIh)
      && sameFrequency(anchor.dHs.value, dHs))
  ...
  if (kicker) kicker.textContent = "R_LOC ACROSS SETTINGS · FREQUENCIES FROM DECISION SCOPE";
  const desc = svg.querySelector("#setting-desc");
  if (desc && !desc.textContent?.includes("exposure frequencies are held")) {
    desc.textContent = `${desc.textContent ?? ""} The two link exposure frequencies are held at the declared decision scope; named markers with different frequencies are omitted from this slice.`;
  }
  ...
  note.innerHTML = `<strong>Frequency slice:</strong> both link exposure frequencies are held at the declared decision scope, ${escapeHtml(scope.shortLabel)} (${formatFrequency(dIh)} index-to-household; ${formatFrequency(dHs)} household-to-social). ${hiddenLabels.length ? `Anchors with different frequencies are omitted from this surface: ${escapeHtml(hiddenLabels.join(", "))}.` : "All named anchors share this frequency slice."}`;
}
