...
import type { AnchorSettingId, DesignGridPoint, EnvelopeV1, ModelOutputsV1, ScenarioV1, SettingId } from "../model/types";
...
export interface PresentationModel {
  ...
  candidate: { label: string; schedule: string; assessment: string };
  probe: { label: string; value: number | null };
...
export function buildPresentation(outputs: ModelOutputsV1): PresentationModel {
  const value = outputs.metrics.rLocEnvelopeMax;
  ...
  return {
    ...
    candidate: {
      label: PRODUCT_LABELS[outputs.scenario.vaccine.id],
      schedule: scheduleLabel(outputs.scenario),
      assessment: `${outputs.scenario.schedule.assessmentLagDays} days after the last dose`
    },
    probe: {
      label: settingLabel(outputs.scenario.setting.id),
      value: outputs.metrics.rLocSelectedSetting
    },
    frontier: {
...
function settingLabel(id: SettingId): string {
